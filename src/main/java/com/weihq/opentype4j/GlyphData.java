...
public class GlyphData extends AbstractParser<GlyphData> {
    ...
    public Path getPath(double x, double y, double fontSize,String colour) {
        Path path = new Path().parse((ScriptObjectMirror) scriptObjectMirror.callMember("getPath", x, y, fontSize,null,null,colour));
        path.setWidth(font.getHead().getxMax() - font.getHead().getxMin());
        path.setHeight(font.getHead().getyMax() - font.getHead().getyMin());
        return path;
    }
